...
## CodeRabbit Follow-Up Fixes (Session 2)

Applied additional fixes from the CodeRabbit review and polish pass:

### Backend
- **ProfileSettings.vue**: Added `deleteError` ref with error display in delete confirmation dialog; fixed stale password error by calling `cancelPassword()` when opening password edit
- **GameController**: Added acronym_length cross-validation (`acronym_length_min` ≤ `acronym_length_max`)
- **GameProcessor**: Capped bot vote max delay to avoid exceeding vote timer
- **robots.txt**: Added `Disallow: /admin` and `Disallow: /debug`
- **api.js**: Changed 404 fallback to generic `notFound` message
- **gameStore.js**: Preserved chat messages across rounds (only clear on full game leave)
- **AppLayout.vue**: Hoisted `usePage()` to setup scope
- **max_players default**: Fixed `?? 10` → `?? 8` across 5 files (GameController, JoinGameAction, Games.vue, web.php)

### Removed "Add Bots" from Game Creation
- Removed bot checkbox widget from `CreateGame.vue` (bots are now managed in lobby only)
- Removed `add_bots` validation from `CreateGameRequest`
- Removed `add_bots` handling from `GameController::store()`
- Rewrote `BotFeatureTest.php` to test via lobby `POST /games/{code}/add-bot` endpoint

---

## Private Game Indicator

Added visual indicators throughout the UI when a game is password-protected:

### GameLayout navbar
- Lock icon (amber) displayed next to `#CODE` when game has a password
- New `isPrivate` prop on `GameLayout.vue`

### Lobby title
- Changed from "Spillkode" to "Privat spill" with lock icon for private games
- Falls back to "Spillkode" for public games

### Game.vue & GameSpectate.vue
- Both pass `:is-private="gameStore.currentGame?.has_password === true"` to GameLayout

---

## Host Live Controls During Play

Hosts can now toggle chat and visibility during active gameplay (not just in lobby).

### Backend
- Modified `GameController::updateSettings()` to allow `chat_enabled` and `is_public` changes outside lobby
- Lobby-only settings are rejected during play with a 422 error
- `updateVisibility()` endpoint had no lobby restriction (already worked)

### Frontend (Game.vue timer bar)
- Added two toggle buttons visible only to hosts:
  - **Chat toggle**: green when enabled, gray when disabled; calls `gameStore.updateSettings()` with `{ chat_enabled }`
  - **Visibility toggle**: green with globe icon when public, amber with lock icon when private; calls `gameStore.updateVisibility()`
- Added `handleToggleChat()` function

### Translations added
| Key | NO | EN |
|-----|----|----|
| `game.disableChat` | Slå av chat | Disable chat |
| `game.enableChat` | Slå på chat | Enable chat |
| `game.makePrivate` | Gjør privat | Make private |
| `game.makePublic` | Gjør offentlig | Make public |

---

## UI Polish

### Lock icon contrast fix (Games.vue)
- Replaced emoji `🔒` inside orange `Badge` (poor contrast) with SVG lock icon
- New styling: `bg-amber-100 text-amber-700` / `bg-amber-900/50 text-amber-300` (dark mode)
- Applied to both "My games" and "Open games" lists

### Vote count on results page
- Changed from `text-xs text-slate-400` (faded, small) to `text-sm font-bold text-emerald-600` (prominent, accent green)
...
| Suite | Tests | Assertions |
|-------|-------|------------|
| PHPUnit (backend) | 251 passed, 1 skipped | ~980 |
| Vitest (frontend) | 76 passed | — |
| **Total** | **327** | — |
...
Host live controls, private game indicators, and UI polish complete.
